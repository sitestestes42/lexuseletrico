...
var orderStatusEnum = pgEnum("order_status", ["pending", "paid", "failed", "cancelled"]);
...
async function getOrderByIdempotencyKey(userId, idempotencyKey) {
  const db = await getDb();
  if (!db) return void 0;
  const result = await db.select().from(orders).where(and(eq(orders.userId, userId), eq(orders.idempotencyKey, idempotencyKey))).limit(1);
  return result[0];
}
async function insertOrderWithItems(input) {
  const db = await getDb();
...
// shared/catalog.ts
var PRODUCT_ID = "inow-1000w";
var productCatalog = {
  id: PRODUCT_ID,
  name: "Bicicleta El\xE9trica INOW 1000W",
  slug: "bicicleta-eletrica-inow-1000w",
  description: "Motor com 1000 W de pot\xEAncia que permite atingir velocidade m\xE1xima de 45 km/h para deslocamentos r\xE1pidos na cidade.",
  originalPriceCents: 659050,
  promotionalPriceCents: 155e3,
  variants: {
    Preto: [
      {
        src: "https://i.postimg.cc/26s1r0F3/inow-preta.png",
        alt: "Bicicleta el\xE9trica INOW 1000W na cor preta"
      }
    ],
    Caramelo: [
      {
        src: "https://i.postimg.cc/QMM9b9dX/inow-caramelo-2.webp",
        alt: "Bicicleta el\xE9trica INOW 1000W na cor caramelo, vista principal"
      },
      {
        src: "https://i.postimg.cc/QxRTjxmT/inow-caramelo.webp",
        alt: "Bicicleta el\xE9trica INOW 1000W na cor caramelo, vista adicional"
      }
    ]
  },
  specifications: [
    { label: "Motor", value: "1000 W" },
    { label: "Velocidade m\xE1xima", value: "45 km/h" },
    { label: "Bateria", value: "\xCDon de l\xEDtio 48V / 15.6 Ah" },
    { label: "Aro", value: "20" },
    { label: "Capacidade", value: "At\xE9 200 kg de peso total" }
  ]
};
var STORE_NAME = "Lexus El\xE9tricos";
var cartMaxQuantity = 10;
function isProductVariant(value) {
...
function calculateServerTotal(lines) {
...
var productPath = `/produto/${productCatalog.slug}`;
var allowedImageUrls = Object.values(productCatalog.variants).flat().map((image) => image.src);
...
// server/routers.ts
var orderLineSchema = z2.object({
  productId: z2.literal(PRODUCT_ID),
  variant: z2.enum(["Preto", "Caramelo"]),
  quantity: z2.number().int().min(1).max(10)
});
var createOrderSchema = z2.object({
  lines: z2.array(orderLineSchema).min(1).max(2),
...
var appRouter = router({
  ...
  orders: router({
    create: protectedProcedure.input(createOrderSchema).mutation(async ({ ctx, input }) => {
      ...
      if (totalCents <= 0) throw new Error("N\xE3o foi poss\xEDvel validar o valor do pedido.");
      ...
      if (existing) {
        return {
          ...
          message: "Este pedido j\xE1 foi validado. Nenhum pedido duplicado foi criado."
        };
      }
